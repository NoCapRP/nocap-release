    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                if vehicle ~= targetVehicle then
                    ...
                    if GetDistanceBetweenCoords(towPos.x, towPos.y, towPos.z, targetPos.x, targetPos.y, targetPos.z, true) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", "Putting the vehicle on it.", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                QBCore.Functions.Notify("Bring the vehicle to the hayens depot!", "success", 5000)
                            end
                            QBCore.Functions.Notify("Vehicle lifted!")
                        end, function() -- Cancel
                            StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", "Remove the vehicle..", 5000, false, true, {
                ...
                CurrentTow = nil
                QBCore.Functions.Notify("Removing the vehicle!")
            end, function() -- Cancel
                StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
        ...
        QBCore.Functions.Notify("You need to be in a tow truck first..", "error")
    end
end)
...
function deliverVehicle(vehicle)
    ...
    QBCore.Functions.Notify("You can go pick up a new vehicle")
