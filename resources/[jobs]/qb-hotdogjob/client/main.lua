...
function UpdateBlip()
    Citizen.CreateThread(function()
        ...
        SetBlipScale(HotdogBlip, 0)
        SetBlipAsShortRange(HotdogBlip, true)
        SetBlipColour(HotdogBlip, 0)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Hotdog")
                ...
                if distance < 10 then
                    ...
                    if not IsWorking then
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Hotdog")
                        ...
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Stop")
                            if IsControlJustPressed(0, Keys["E"]) then
                                StopWorking()
                            end
                        elseif distance < 3 then
...
function StartWorking()
    QBCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            ...
            UpdateUI()
            CheckLoop()
            QBCore.Functions.Notify(' € 1000 Deposit received!..', 'success')
        else
            QBCore.Functions.Notify('You have no money..', 'error')
        end
    end)
end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Hold vehicle')
                            if IsControlJustPressed(0, Keys["E"]) then
                                TakeHotdogStand()
                            end
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Drop the vehicle')
                            if IsControlJustPressed(0, Keys["E"]) then
                                LetKraamLose()
                            end
                        end
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Hold the vehicle')
                    end
                end
            else
                break
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if SellingData.Enabled then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Prepare a hot dog sandwich [Sales: ~g~Active~w~]')
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Prepare a hot dog sandwich [Satis: ~r~Closed~w~]')
                        end
                        if IsControlJustPressed(0, Keys["E"]) then
                            StartHotdogMinigame()
                        end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            QBCore.Functions.Notify(' You are so far from the Hot Dog Stand..', 'error')
        end
    else
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            QBCore.Functions.Notify('You are far from your vehicle ..', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sell '..HotdogsForSale..'in front of €'..(HotdogsForSale * SellingPrice)..',- / [8] Decline')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    QBCore.Functions.Notify(HotdogsForSale..'Hotdog Price: €'..(HotdogsForSale * SellingPrice)..',-', 'success')
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'No stock .. / [8] Reject customer')
            ...
            QBCore.Functions.Notify('You got too far from the vehicle..', 'error')
            break
        end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            QBCore.Functions.Notify('You did '..Config.Stock[Quality].Label..' Hotdog!')
            ...
            if Luck == LuckyNumber then
                QBCore.Functions.Notify(' '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog done!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                QBCore.Functions.Notify(' '..Config.Stock[Quality].Label..' You made a hotdog !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        QBCore.Functions.Notify('Not enough stock ('..Config.Stock[Quality].Label..')stock more than that..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                QBCore.Functions.Notify('1000 € You got your deposit back!', 'success')
            else
                QBCore.Functions.Notify('Some things went wrong..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Your hot dog counter was nowhere to be seen, you did NOT get your deposit back!!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('qb-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            QBCore.Functions.Notify('Hot dog modu kaldırıldı!')
        end
    end
end)
