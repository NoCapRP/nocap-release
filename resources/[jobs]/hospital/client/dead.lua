	...
	while true do
		...
		if isDead then
            ...
            if not isInHospitalBed then 
                if deathTime > 0 then
                    DrawTxt(0.89, 1.44, 1.0,1.0,0.6, "RESPAWN IN: ~b~" .. math.ceil(deathTime) .. "~w~ SECONDS", 255, 255, 255, 255)
                else
                    DrawTxt(0.89, 1.44, 1.0,1.0,0.6, "~w~ KEEP ~b~E ~w~PRESSED TO RESPAWN ($"..Config.BillCost..")", 255, 255, 255, 255)
                end
            end
