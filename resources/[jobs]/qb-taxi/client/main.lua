    ...
    if whitelistedVehicle() then
        if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                QBCore.Functions.Notify('De NPC staat aangegeven op uw navigatie!', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    QBCore.Functions.Notify('Bring the npc to the marked location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                QBCore.Functions.Notify('Your already doing a npc mission..')
            end
        else
            QBCore.Functions.Notify('There are non npc availble..')
        end
    else
        QBCore.Functions.Notify('Your not in a cab :(')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        QBCore.Functions.Notify('Deliverd the person', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            QBCore.Functions.Notify('This vehicle does not have a cap meter..', 'error')
        end
    else
        QBCore.Functions.Notify('Your not in a vehicle..', 'error')
    end
end)
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify('The cap meter is not active..', 'error')
    end
end)
...
AddEventHandler('qb-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify('No cap meter to see..', 'error')
    end
end)
