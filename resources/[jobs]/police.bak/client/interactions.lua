Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
			...
			DisableControlAction(0, 71, true) -- Disable qbving forward in vehicle
...
RegisterNetEvent('police:client:SeizeqbverLicense')
AddEventHandler('police:client:SeizeqbverLicense', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("police:server:SeizeqbverLicense", playerId)
    else
        QBCore.Functions.Notify("No one around!", "error")
    end
end)
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", "Steal items..", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            QBCore.Functions.Notify("Time must be greater than 0..", "error")
        end
    else
        QBCore.Functions.Notify("No one around!", "error")
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                QBCore.Functions.Notify("You cannot cuff in a vehicle", "error")
            end
        else
            QBCore.Functions.Notify("No one around!", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    QBCore.Functions.Notify("You have no handcuffs with you", "error")
                end
            end, "handcuffs")
        else
            QBCore.Functions.Notify("No one around!", "error")
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("You are captivated!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("You are now cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
