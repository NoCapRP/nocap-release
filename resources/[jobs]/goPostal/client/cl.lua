   ...
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -402.732, 6172.939, 31.532, true) < 15) then
	DrawMarker(23, -402.732, 6172.939, 31.532-0.9, 0, 0, 0, 0.0, 0, 0, 3.0, 3.0, 0.5001, 255, 255, 255, 255, 0, 0, 0, 0)
	   ...
	   TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(goPostalVehicle))
		...
		QBCore.Functions.Notify('Fill your truck at parking lot', 'success')
      end
     end
	end
   end
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bootPos.x, bootPos.y, bootPos.z, true) < 2.6 then 
    ...
    if IsControlJustPressed(0, 38) then 
     ...
     if restockPackages == 0 then 
      ...
      newShift()
	  onJob = true
	  --TriggerEvent('notifications', "#000080", "Go Postal", "Van Restocked, Begin Delivering Your Packages")
	  QBCore.Functions.Notify('Van restocked, deliver packages', 'success')
