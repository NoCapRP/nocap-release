        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
					...
					Player.Functions.AddMoney('cash', math.random(2000, 3000), "robbery-bank")
			...
			--Player.Functions.AddMoney('cash', math.random(3000, 4000), "robbery-bank")
        end
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    -- ply.Functions.AddItem(item.item, itemAmount)
                    -- TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.item], "add")
					Player.Functions.AddMoney('cash', math.random(4000, 5000), "robbery-bank")
                elseif Config.RewardTypes[itemType].type == "money" then
                ...
                -- local info = {
                    -- crypto = math.random(0, 0)
                -- }
                -- ply.Functions.AddItem("cryptostick", 1, false, info)
                -- TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['cryptostick'], "add")
				Player.Functions.AddMoney('cash', math.random(5000, 6000), "robbery-bank")
            end
        else
