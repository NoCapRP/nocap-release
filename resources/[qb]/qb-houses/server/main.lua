...
AddEventHandler('qb-houses:server:removeHouseKey', function(house, citizenData)
	...
	TriggerClientEvent('QBCore:Notify', src, citizenData.firstname .. " " .. citizenData.lastname .. "'Your keys has been removed..", 'error', 3500)
	QBCore.Functions.ExecuteSql(false, "UPDATE `player_houses` SET `keyholders` = '"..json.encode(housekeyholders[house]).."' WHERE `house` = '"..house.."'")
end)
...
QBCore.Commands.Add("decorate", "Decorate :)", {}, false, function(source, args)
...
AddEventHandler('qb-houses:server:giveHouseKey', function(target, house)
	...
	if tPlayer ~= nil then
		if housekeyholders[house] ~= nil then
			...
			TriggerClientEvent('QBCore:Notify', tPlayer.PlayerData.source, 'You have the keys to '..Config.Houses[house].adress..' !', 'success', 2500)
		else
...
QBCore.Commands.Add("createhouse", "Create a house", {{name="price", help="Price"},{name="tier", help="Tier"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("addgarage", "Add a garage to the closest house", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "realestate" then
...
QBCore.Commands.Add("ring", "Ring", {}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
