...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local vehicleCoordsa = GetEntityCoords(vehicled)

        local closestVehicle = QBCore.Functions.GetClosestVehicle()

        local Distance = QBCore.Functions.GetClosestVehicle()

        local vehicleCoords = GetEntityCoords(vehicled)

        local dimension = GetModelDimensions(GetEntityModel(closestVehicle), First, Second)
         --QBCore.Functions.Notify(dimension, 'error')
       
        if DoesEntityExist(vehicled) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoordsa) < 2.5 then
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    DrawText3D2(Vehicle.Coords.x, Vehicle.Coords.y, Vehicle.Coords.z+0.1, 'Press [~g~SHIFT~w~] and [~g~E~w~] to push the vehicle')
                end
