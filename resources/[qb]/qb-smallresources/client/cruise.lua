...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1)) then
            ...
            if IsControlJustReleased(0, Keys["G"]) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then 
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    cruiseSpeed = Speed
                    if cruiseOn then
                        QBCore.Functions.Notify("Limiter switched off!")
                    else
                        QBCore.Functions.Notify("Limiter set to "..tostring(math.floor(cruiseSpeed * 3.6)).."km/h")
                    end
