                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe].x, Config.Safes[safe].y, Config.Safes[safe].z, '~g~E~w~ - Attempt code lock')
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    QBCore.Functions.Notify("Not enough cops.. (3 required)", "error")
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe].x, Config.Safes[safe].y, Config.Safes[safe].z, 'Save opened..')
                        end
                    end
                end
            end
                ...
                if usingAdvanced then
                    ...
                    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                        if result then
                            ...
                            QBCore.Functions.Notify("It looks like u need a screwdriver..", "error")
                        end
                    end, "screwdriverset")
                end
...
AddEventHandler('qb-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if (PlayerJob.name == "police" or PlayerJob.name == "police1" or PlayerJob.name == "police2" or PlayerJob.name == "police3" or PlayerJob.name == "police4" or PlayerJob.name == "police5" or PlayerJob.name == "police6" or PlayerJob.name == "police7" or PlayerJob.name == "police8") and PlayerJob.onduty then
        ...
        AddTextComponentString("112: Store robbery")
