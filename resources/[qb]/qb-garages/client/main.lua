            ...
            if Engine <= 50.0 then
                QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                    QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                        ...
                        QBCore.Functions.Notify("Vehicle Out: Engine: " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
                ...
                QBCore.Functions.Notify("You can\'t duplicate this vehicle")
            end
        else
            QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
...
function DepotLijst()
    QBCore.Functions.TriggerCallback("qb-garage:server:GetDepotVehicles", function(result)
        ...
        if result == nil then
            QBCore.Functions.Notify("There are no vehicles in the depot", "error", 5000)
            closeMenuFull()
        else
...
function TakeOutVehicle(vehicle)
    ...
    elseif vehicle.state == "In Beslag" then
        QBCore.Functions.Notify("This vehicle has been seized by the Police", "error", 4000)
    end
end
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                QBCore.Functions.Notify("No one owns this vehicle...", "error", 3500)
                            end
                        end, plate)
                    end
                end
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        ...
                                        QBCore.Functions.Notify("Vehicle parked in "..HouseGarages[currentHouseGarage].label, "primary", 4500)
                                    else
                                        QBCore.Functions.Notify("No one owns this vehicle...", "error", 3500)
                                    end
