				...
				if IsPedInAnyVehicle(GetPlayerPed(-1)) then
					...
					if vehicle ~= 0 and vehicle ~= nil then 
						...
						if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 2.5 and not isBusy then
							DrawText3Ds(vehpos.x, vehpos.y, vehpos.z, "~g~E~w~ - Scrapping the vehicle")
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										ScrapVehicle(vehicle)
									else
										QBCore.Functions.Notify("This vehicle can\'t be scrapped..", "error")
									end
								else
									QBCore.Functions.Notify("You're not the driver..", "error")
								end
							end
						end
					end
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		SetTimeout(math.random(20000, 60000), function()
			...
			TriggerServerEvent('qb-phone:server:sendNewMail', {
				...
				message = "You can only Destroy a few vehicles.<br /><br /><strong>Vehicle list:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
		QBCore.Functions.Notify("You are not allowed to Destroy vehicles now..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", "Scrapping vehicle..", scrapTime, false, true, {
