    ...
    QBCore.Functions.TriggerCallback('qb-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            for _, zone in pairs(Config.CornerSellingZones) do
                ...
                if zoneDist < 500 then
                    if not cornerselling then
                        cornerselling = true
                        QBCore.Functions.Notify('Corner selling mode enabled..')
                        startLocation = GetEntityCoords(GetPlayerPed(-1))
                        -- TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
                    else
                        cornerselling = false
                        QBCore.Functions.Notify('Corner selling position disabled.')
                        -- ClearPedTasks(GetPlayerPed(-1))
                    end
                else
                    QBCore.Functions.Notify('You are too away from allowed selling zones.','error')
                end
            end
        else
            QBCore.Functions.Notify('You dont have drugs on you..', 'error')
        end
    end)
end)

function toFarAway()
    QBCore.Functions.Notify('You walked away. Failed', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                QBCore.Functions.Notify('You have been robbed of - $'..bagAmount..' bag(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        QBCore.Functions.Notify('Declined!', 'error')
                        hasTarget = false
